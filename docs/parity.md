...
| `getDecision(slotId)` | WS → long-poll → short-poll ladder | **`#await_decision(slot_id)`** is the equivalent — same ladder, same budgets. `#poll_decision_once(slot_id)` is a *single* long-poll request. Upstream keeps its equivalent (`pollDecisionOnce`) private; we expose it because it wraps a documented canonical endpoint 1:1, renamed away from `decision` so it cannot be mistaken for `getDecision` | adapt per #001 |
